"""
兼容层：alphaagent.scenarios.qlib.experiment

AlphaAgent 目前缺少原 RD-Agent 中的 `scenarios.qlib.experiment` 包，
但项目内仍依赖诸如:
 ...
 - alphaagent.scenarios.qlib.experiment.model_experiment.QlibModelScenario
 等类。

这里提供一个兼容层：
1. 优先从 AlphaAgent 自己的 experiment 子模块加载（如果你之后真正迁移了这些文件）。
2. 如果没有本地实现，则自动回退到仓库中的 RD-Agent 版本
   （路径为 `wuyinze/RD-Agent/rdagent/...`），并将其作为 `rdagent` 包挂载到 sys.path。
"""
...
# 尝试优先从 AlphaAgent 自己的 experiment 子模块加载，如果不存在，再降级到 RD-Agent 版本
def _lazy_import(module_name: str):
    """
    延迟导入工具：
    1. 先尝试 alphaagent.scenarios.qlib.experiment.<module_name>
    2. 如果失败，再尝试 rdagent.scenarios.qlib.experiment.<module_name>
    """
    base_paths = [
        "alphaagent.scenarios.qlib.experiment",
