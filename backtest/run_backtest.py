...
使用方式:
    # 使用 Alpha158(20) 因子库
    python backtest/run_backtest.py -c backtest/config.yaml --factor-source alpha158_20
    
    # 使用 Alpha158 完整因子库
    python backtest/run_backtest.py -c backtest/config.yaml --factor-source alpha158
    
    # 使用 Alpha360 因子库
    python backtest/run_backtest.py -c backtest/config.yaml --factor-source alpha360
    
    # 使用自定义因子库
    python backtest/run_backtest.py -c backtest/config.yaml --factor-source custom \
        --factor-json /path/to/factors.json
    
    # 组合使用官方因子和自定义因子
    python backtest/run_backtest.py -c backtest/config.yaml --factor-source combined \
        --factor-json /path/to/factors.json

    # 指定测试时间段
    python backtest/run_backtest.py -c backtest/config.yaml --test-period 2023
    # 可选时间段: default(2022-2025), 2021, 2022, 2023, 2024, 2025, 2022-2023, 2024-2025

    # 仅计算 IC 指标（不运行策略组合回测，速度更快）
    python backtest/run_backtest.py -c backtest/config.yaml --ic-only
"""
