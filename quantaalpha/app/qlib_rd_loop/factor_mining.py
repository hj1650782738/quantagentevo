...
import os
import pickle
from quantaalpha.app.qlib_rd_loop.conf import ALPHA_AGENT_FACTOR_PROP_SETTING
from quantaalpha.app.qlib_rd_loop.planning import generate_parallel_directions
from quantaalpha.app.qlib_rd_loop.planning import load_run_config
from quantaalpha.components.workflow.quantaalpha_loop import QuantaAlphaLoop
from quantaalpha.components.evolution import (
    EvolutionController, 
    EvolutionConfig,
    StrategyTrajectory,
    RoundPhase,
...
from quantaalpha.oai.llm_conf import LLM_SETTINGS
...
def main(path=None, step_n=100, direction=None, stop_event=None, config_path=None, evolution_mode=None):
    ...
    .. code-block:: python
        ...
        dotenv run -- python rdagent/app/qlib_rd_loop/factor_quantaalpha.py $LOG_PATH/__session__/1/0_propose  --step_n 1  --potential_direction "[Initial Direction (Optional)]"
    ...
    try:
        ...
        # 传统模式（无进化）
            ...
            model_loop = QuantaAlphaLoop.load(path, use_local=use_local)
            model_loop.run(step_n=step_n, stop_event=stop_event)
    except Exception as e:
